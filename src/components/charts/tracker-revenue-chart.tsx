...
interface TrackerRevenueChartProps {
  data: ChartDataPoint[] | null;
}

const formatCurrency = (value: number) => `R$${value.toFixed(0)}`;

export function TrackerRevenueChart({ data }: TrackerRevenueChartProps) {
  ...
  return (
    <ResponsiveContainer width="100%" height={350}>
      <BarChart data={data} margin={{ top: 30, right: 0, left: 0, bottom: 0 }}>
        ...
        />
        <Bar dataKey="revenue" name="Receita" fill="hsl(221.2 83.2% 53.3%)" radius={[4, 4, 0, 0]}>
          <LabelList dataKey="revenue" position="top" formatter={formatCurrency} style={{ fontSize: '12px', fill: 'hsl(221.2 83.2% 53.3%)' }} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
