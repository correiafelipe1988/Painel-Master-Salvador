// Script para executar análise maio-julho usando Firebase client-side
...
const firebaseConfig = {
  ...
  storageBucket: "motosight-dashboard.firebasestorage.app",
...
async function executarAnaliseRealMaioJulho() {
  try {
    console.log('🔥 Conectando ao Firebase (client-side)...');
...
function analisarCrescimentoMaioJulho(motorcycles) {
  ...
  motorcycles.forEach(moto => {
    if (moto.placa && moto.data_criacao) {
      ...
      let date;
      if (moto.data_criacao.toDate) {
        // Firestore Timestamp
        date = moto.data_criacao.toDate();
      } else if (moto.data_criacao.seconds) {
        // Firestore Timestamp objeto
        date = new Date(moto.data_criacao.seconds * 1000);
      } else {
        // String de data
        date = new Date(moto.data_criacao);
      }
